...
control "KEYC-01-000048" do
  ...
  desc  "rationale", ""
  desc  "check", "
    If Keyclaok rely on directory services for user account management, this is not applicable and the connected directory services must perform this function. 
    
    Verify Keycloak are configured to notify the system administrators and ISSO when accounts are modified.
    
    If Keycloak are not configured to notify the system administrators and ISSO when accounts are modified, this is a finding.
